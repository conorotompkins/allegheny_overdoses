...
df_geo_zip <- df_geo_zip %>% 
  ...
  mutate(od_per_capita = (n / estimate) * 1000,
         od_per_capita = case_when(estimate <= 400 ~ 0,
                                   estimate > 499 ~ od_per_capita))

threshhold <- 400
df_geo_zip %>% 
  ggplot() +
  geom_sf(aes(fill = od_per_capita), color = NA) +
  geom_sf(data = df_geo_county, color = "red", size = 1, alpha = 0) +
...
df_demo %>% 
  ...
  facet_wrap(~sex)
```
need different intervention techniques for different population
```{r}
df_demo %>% 
