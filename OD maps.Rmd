...
counties %>% 
  filter(state == "PA",
         county_name %in% c("Allegheny County", "Washington County", "Butler County", "Westmoreland County", "Beaver County", "Armstrong County")) %>% 
...
df_geo_od %>%
  ...
  ggplot() +
  geom_sf(aes(fill = n), size = .1, color = "grey") +
  geom_sf(data = df_geo_county, color = "red", size = 1, linetype = 1, alpha = 0) +
